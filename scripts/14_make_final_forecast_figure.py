...
FORECASTS_FILE = RESULTS_DIR / "forecasts" / "all_model_forecasts_final.csv"
...
def plot_one_panel(ax, series_id: str, history_df: pd.DataFrame, truth_df: pd.DataFrame, forecast_df: pd.DataFrame) -> None:
    ...
    ax.plot(history_df["ds"], history_df["y"], linewidth=2.0, marker="o", markersize=3.5, label="Past history")
    ax.plot(truth_df["ds"], truth_df["y"], linewidth=2.0, linestyle="--", marker="o", markersize=3.5, label="Observed future")

    forecast_origin = truth_df["ds"].iloc[0]
    ax.axvline(forecast_origin, linestyle="--", linewidth=1.2, alpha=0.8)
...
def main() -> None:
    ...
    for ax, series_id in zip(axes, FOCUS_SERIES):
        ...
        plot_one_panel(ax, series_id, history_df, truth_df, forecast_sub)
    ...
    output_path = FIGURES_DIR / "final_fig_forecast_comparison_paper.png"
