...
LATENT_CONFIG_FILE = CONFIGS_DIR / "latent_shared.yaml"
...
def build_latent_shared_model(
    ...
    checkpoint_path = MODELS_DIR / "latent_shared.pt"
...
def main() -> None:
    ...
    latent_config = load_yaml(LATENT_CONFIG_FILE)
    ...
    latent_model = build_latent_shared_model(data_config, latent_config, device)
    ...
    for model_name, model in model_specs:
        ...
        model_output_path = FORECASTS_DIR / f"{model_name}_forecasts.csv"
    ...
    all_output_path = FORECASTS_DIR / "all_model_forecasts.csv"
    all_forecasts.to_csv(all_output_path, index=False)

    print(f"\nSaved combined forecasts: {all_output_path}")
    print("\n=== FIRST 18 ROWS OF COMBINED FORECASTS ===")
